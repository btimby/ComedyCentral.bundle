...
def FeedMenu(title, url, thumb=''):
    ...
    if not thumb:
        try: thumb=thumb = HTML.ElementFromString(content).xpath('//meta[@property="og:image"]/@content')[0].strip()
        except: thumb=''
    ...
    for zone in zone_list:
        ...
        # ent_m013 and f1013 - each show's full episodes, ent_m020-playlists, ent_m160-Standup video clips, and ent_m012-standup specials listing
            ...
            if ent_code=='ent_m012':
                ...
                if ('/video-clips/') not in example_url and ('/full-episodes/') not in example_url:
                    continue
            oc.add(DirectoryObject(key=Callback(ShowVideos, title=title, url=json_feed, result_type=result_type),
                title=title,
                thumb=Resource.ContentsOfURLWithFallback(url=thumb)
            ))
        ...
        if ent_code=='ent_m081':
            for item in json['result']['shows']:
                oc.add(DirectoryObject(key=Callback(ShowVideos, title=item['show']['title'], url=item['fullEpisodesFeedURL'], result_type='episodes'),
...
def ShowVideos(title, url, result_type):
    ...
    for video in videos:
        # VIDEOS ARE LOCKED ON WEBSITE NOT THE BACK END SO ALL VIDEOS WORK IN PLEX URL SERVICE
...
def Search(title, url, start=0, search_type=''):
    ...
    for item in json['response']['docs']:
        ...
        if result_type=='Series':
            oc.add(DirectoryObject(key=Callback(FeedMenu, title=item['title_t'], url=item_url, thumb=item['imageUrl_s']),
...
def GetType(ent):
    ...
    ENTTYPE_LIST = [{'ent':'ent_m071', 'type':'sortingOptions'},{'ent':'f1071', 'type':'sortingOptions'},{'ent':'ent_m013', 'type':'episodes'}, {'ent':'f1013', 'type':'episodes'},{'ent':'ent_m081', 'type':'episodes'},{'ent':'ent_m069', 'type':'shows'},{'ent':'ent_m157', 'type':'promo'},{'ent':'ent_m020', 'type':'playlist'},{'ent':'ent_m160', 'type':'items'}]
